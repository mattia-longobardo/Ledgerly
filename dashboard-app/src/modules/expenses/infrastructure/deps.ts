 ...
 */
export function expenseDeps(
  tx: DbClient,
  requestId?: string | null,
): UseCaseDeps & { links: ProviderLinksRepository } {
  return {
    ...
    // Widening the return type rather than `UseCaseDeps` itself: only
    // `updateCategory` (and the sync, which is handed its own) reads links, so
    // every other use case keeps the narrower bag it declares.
