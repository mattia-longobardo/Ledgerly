...
import { syncIntegrationAction } from "@/app/actions/integrations";
...
export interface AccountsToolbarProps {
  ...
  /** "header" adds the Sync action; "empty" links to connect Wallet in Settings instead. */
...
export function AccountsToolbar({
  ...
  function sync() {
    ...
    startTransition(async () => {
      ...
      data.append("provider", "wallet");
      const result = await syncIntegrationAction(data);
      if (!result.ok) {
        setError(result.error);
        return;
      }
      // The reconciliation counts now live on the run, not on the action's
      // result — the toast reports what actually happened: the sync finished.
      setToast(`Sync finished: ${result.data.kind} (${result.data.status}).`);
      router.refresh();
    });
  }
    ...
    <span className={cn("flex flex-col items-start gap-2", className)}>
      ...
      <span className="flex flex-wrap items-center gap-2">
        ...
        {variant === "header" ? (
          ...
          <Link href="/settings/integrations/wallet" className={SECONDARY}>
