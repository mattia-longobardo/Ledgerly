...
export { budgetDeps } from "../infrastructure/deps";

/**
 * Test seams for `runForPrincipal`, mirroring the interests and funds
 * modules' `ui/run.ts`. Both are no-ops outside `NODE_ENV=test`, so
...
export function setBudgetDepsFactoryForTests(factory: (() => UseCaseDeps) | null): void {
 ...
 * that touches budgets goes through here — the budgets use cases take a flat
 * deps bag and never open a context themselves.
 */
export async function runForPrincipal<T>(
  ...
  return withUserContext(db, { userId: principal.userId }, (tx) => fn(budgetDeps(tx), principal));
}
