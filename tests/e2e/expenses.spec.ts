...
/**
 * The count and total beside the page title. The month group row and the phone list say the same
 * sentence, so the header is addressed through the heading it sits next to rather than by text.
 */
...
test("synced movements are filtered, recategorised and hidden, and the totals follow", async ({ page }) => {
  await test.step("the month opens with the movements the sync brought in", async () => {
    ...
    await expect(summary(page)).toHaveText("4 transactions · 2.020,51 €");
  });
  ...
  await test.step("hiding a movement takes it out of the totals, and Show hidden brings it back", async () => {
    ...
    await expect(summary(page)).toHaveText("3 transactions · 2.041,51 €");
  ...
  await test.step("sorting puts the order in the URL, so the server renders it", async () => {
    ...
    await expect(page.getByText("4 transactions · 2.020,51 €")).toHaveCount(1);
  });
