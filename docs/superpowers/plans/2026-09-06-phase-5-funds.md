...
- [x] **Step 1: `migrate-funds.ts`** (idempotent; every read/write under `withSystemContext`; template `scripts/migrate-paperless.ts`):
...
- [x] **Commit:** `git add scripts package.json && git commit -m "feat(funds): legacy funds migration and validator"`

---

### Deviation — Task 8

Apply R5-C1/C2: create one nonzero opening `adjustment` from the earliest legacy setting, dated its `effective_from`; plans alone do not contribute actual money. Preserve existing user edits on rerun. The validator independently reconstructs legacy amounts in bigint cents from legacy tables and proves both the old visible timeline (Cometa quarter-end +2) and the authoritative actual-posting timeline (+1), reporting intentional historical differences explicitly. Extend checks through the last expected future posting/visible month as needed so future accrued rows cannot disappear while current totals still match. Do not import deleted calculators or compare the migration transform to itself. `scripts/migrate-paperless.ts` has been retired; use the remaining migration/validation scripts as structural references.
...
- [x] **Step 1:** `npm run typecheck && npm test && npm run test:db:up && npm run test:integration && npm run build && npm run openapi:generate && git diff --exit-code docs/api/openapi.json`. All green; the route table lists `/finance/funds/[id]`.
